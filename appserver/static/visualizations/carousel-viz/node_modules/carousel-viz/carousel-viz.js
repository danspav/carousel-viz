class carousel_viz{
	...
	resize(height){
		...
		if(isVertical && objCarousel.slidesToShow>3){
			...
			verticalContainerHeight = verticalSlideHeight * objCarousel.slidesToShow + 15;
			if (verticalContainerHeight < 165) { verticalContainerHeight= 165;} // Bug: Slides are off set incorrectly at container heights < 165 for 2 or 3 panels
			verticalSlideHeight = Math.ceil((verticalContainerHeight/objCarousel.slidesToShow)-15) // Allow 15px for the dots, allow 5 for borders and padding
			window.jQuery("div#" + this.id).height(verticalContainerHeight);
			window.jQuery("div#" + this.id +" div.singlevaluebox").height(verticalSlideHeight);
			maxFontSize = (.7 * verticalSlideHeight ) - 15; // take 70% of the CSS height for the value div to allow for padding.
