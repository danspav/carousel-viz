class carousel_viz{
	...
	start(){
		...
		var isVertical = (this.orientation=="vertical");
			window.jQuery('#' + this.id).slick({
				...
				adaptiveHeight: false,	// Only used for Vertical Display?
				pauseOnFocus: true,
				focusOnSelect: true,
				cssEase: 'ease'
			});
	...
	resize(height){
		...
		if(isVertical && objCarousel.slidesToShow>3){
			//verticalSlideHeight = Math.ceil(0.9*(height/objCarousel.slidesToShow)) // Allow 15px for the dots, allow 5 for borders and padding
			///window.jQuery("div#" + this.id).height(verticalSlideHeight * objCarousel.slidesToShow + 15);
			//window.jQuery("div#" + this.id +" div.singlevaluebox").height(verticalSlideHeight);
		//	maxFontSize = (.7 * verticalSlideHeight ) - 15; // take 70% of the CSS height for the value div to allow for padding.
			verticalSlideHeight = Math.ceil(0.9*((height/objCarousel.slidesToShow)-5)) // Allow 15px for the dots, allow 5 for borders and padding
			window.jQuery("div#" + this.id).height(0.9*height);
			window.jQuery("div#" + this.id +" div.singlevaluebox").height(verticalSlideHeight);
			maxFontSize = (.7 * verticalSlideHeight ) - 15; // take 70% of the CSS height for the value div to allow for padding.
			
		} else if(isVertical && (objCarousel.slidesToShow==2 || objCarousel.slidesToShow==3)){
			verticalSlideHeight = Math.ceil(0.9*((height/objCarousel.slidesToShow)-5)) // Allow 15px for the dots, allow 5 for borders and padding
			window.jQuery("div#" + this.id).height(0.9*height);
			window.jQuery("div#" + this.id +" div.singlevaluebox").height(verticalSlideHeight);
			maxFontSize = (.7 * verticalSlideHeight ) - 15; // take 70% of the CSS height for the value div to allow for padding.
			...
			console.log("Slide Height=" + verticalSlideHeight);
			console.log("Viz Height=" + (verticalSlideHeight * objCarousel.slidesToShow + 15));
	...
	numberWithCommas(x){
		...
		getHTML(){
			...
			if(this.unitPosition=="before"){
				html += '<div class="value"><span class="unit">' + this.unit + '&thinsp;</span><span class="val" valueField="' + this.valueField + '" rawValue="' + vizUtils.escapeHtml(this.value) + '">' +  this.getValue() +  '</span></div>';
			}else{
				html += '<div class="value"><span class="val" valueField="' + this.valueField + '" rawValue="' + vizUtils.escapeHtml(this.value) + '">' + this.getValue() + '</span><span class="unit">&thinsp;' + this.unit +  '</span></div>';
			}
