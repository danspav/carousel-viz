class carousel_viz{
	...
	setConfig(config, namespace){
		...
		this.blockFill = vizUtils.escapeHtml(config[namespace + "blockFill"]) || true;
		...
		this.defaultCaption = vizUtils.escapeHtml(config[namespace + 'defaultCaption']) || ""; // HTML encoded below
		this.size = vizUtils.escapeHtml(config[namespace + 'size']) || "normal"; // HTML encoded below
		...
		this.blockFill = (this.blockFill || this.blockFill=="true");
	...
	setData(data){
			...
			try{
				...
				for (i=0; i<data.rows.length; i++){
					...
					if (data_item[fields["caption"]]==""){ data_item[fields["caption"]]= this.defaultCaption;}
	...
	start(){
				...
				window.jQuery("section#" + this.id + " div.caption").textfill({allowOverflow:false,changeLineHeight:true,maxFontPixels:18,minFontPixels:7});
