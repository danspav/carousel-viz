class carousel_viz{
	constructor(){
		...
		this.maxFontSizes = {xsmall:70,small:95,normal:145,large:195,xlarge:240};
		this.height = 100;
	}
	...
	start(){
			...
			var maxFontSize = (.8* objCarousel.height * .8); // .8 is the CSS height for the value div, and take 80% of that to allow for padding.
			window.jQuery("div#" + this.id + " div.value").textfill({ allowOverflow: false, changeLineHeight: true, maxFontPixels: maxFontSize, widthOnly: true });
			...
			/*
			// Update carousel text size on div resize
			window.jQuery("div#" + this.id).on('reSize', function (event, slick) {
				window.jQuery("div#" + objCarousel.id + " div.value").textfill({ allowOverflow: false, changeLineHeight: true, maxFontPixels: sizes[objCarousel.size], widthOnly: true });
				window.jQuery("div#" + objCarousel.id + " div.caption").textfill({ allowOverflow: false, changeLineHeight: false, maxFontPixels: 18, minFontPixels: 8 });
			});
			
			//update carousel text size on window resize. Wait 1 second because it doesn't always work if we do it right away.
	...
	resize(height){
		var objCarousel=this;
		objCarousel.height = height;
		var maxFontSize = (.8 * objCarousel.height * .8) - 15; // .8 is the CSS height for the value div, and take 90% of that to allow for padding.
		window.jQuery("div#" + this.id).height(height);
		window.jQuery("div#" + this.id +" div.singlevaluebox").height(.8*height);
				
		setTimeout(function(){ 
			window.jQuery("div#" + objCarousel.id + " div.value").textfill({ allowOverflow: false, changeLineHeight: true, maxFontPixels: maxFontSize,minFontPixels: 12, widthOnly: true });
			window.jQuery("div#" + objCarousel.id + " div.caption").textfill({ allowOverflow: false, changeLineHeight: true, maxFontPixels: 18, minFontPixels: 12,widthOnly: false });
		},1000);
	}
