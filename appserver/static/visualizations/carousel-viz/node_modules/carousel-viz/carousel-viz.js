class carousel_viz{
	...
	resize(height){
		...
		if(isVertical && objCarousel.slidesToShow>1){
			evenHeight = Math.floor(jQuery("div#" + this.id +" div.singlevaluebox").height((.9*height)/objCarousel.slidesToShow -15));
			if(evenHeight%2==1) { evenHeight -=1;}
			window.jQuery("div#" + this.id).height(0.9*height);
			window.jQuery("div#" + this.id +" div.singlevaluebox").height((.9*height)/objCarousel.slidesToShow -15);
			maxFontSize = (.7 * evenHeight ) - 15; // take 70% of the CSS height for the value div to allow for padding.
